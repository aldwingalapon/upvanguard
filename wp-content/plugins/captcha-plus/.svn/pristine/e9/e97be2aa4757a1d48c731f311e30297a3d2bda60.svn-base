	...
	class Cptchpls_Whitelist extends WP_List_Table {
		...
		*/
		...
		function display_content() {
							...
							<td>
								...
								<div class="bws_help_box dashicons dashicons-editor-help cptchpls_thumb_block">
									<div class="bws_hidden_help_text" style="width: 200px;">
										<p><?php printf( __( 'With this option, CAPTCHA will not be displayed for IP-addresses from the whitelist of %s', 'captcha-plus' ), $limit_attempts_info['name'] ); ?></p>
			...
			<?php } ?>
			<form class="form-table cptchpls_whitelist_form" method="post" action="admin.php?page=captcha-plus.php&amp;action=whitelist" style="margin: 10px 0;<?php echo ! ( isset( $_REQUEST['cptchpls_show_whitelist_form'] ) || isset( $_REQUEST['cptchpls_add_to_whitelist'] ) ) ? 'display: none;': ''; ?>">
		...
		*/
		function prepare_items() {
			...
			$this->per_page    = $this->get_items_per_page( 'cptchpls_per_page', 20 );
		...
		*/
		...
		function column_ip( $item ) {
			...
			$url      = "?page=captcha-plus.php&action=whitelist&cptchpls_remove={$item['id']}{$order_by}{$order}{$paged}{$s}";
			$actions = array(
				'remove' => '<a href="' . wp_nonce_url( $url, "cptch_nonce_remove_{$item['id']}" ) . '">' . __( 'Remove from whitelist', 'captcha-plus' ) . '</a>'
			);
		...
		function get_limit_attempts_info() {
			...
			if ( 'actived' == $this->la_info['status'] ) {
				...
				$data = array(
					...
					'notice'           => sprintf( __( 'you should install %s to use this functionality', 'captcha-plus' ), '<a href="http://bestwebsoft.com/products/limit-attempts?k=d70b58e1739ab4857d675fed2213cedc&pn=75&v=' . $cptchpls_plugin_info["Version"] . '&wp_v=' . $wp_version . '" target="_blank">Limit Attempts by BestWebSoft</a>' ),
					'disabled'         => true,
				);
			}
			return $data;
