...
if ( file_exists( dirname( __FILE__ ) . '/includes/envato-update.php' ) )
	require_once( dirname( __FILE__ ) . '/includes/envato-update.php' );

if ( ! function_exists( 'cptchpls_admin_menu' ) ) {
	function cptchpls_admin_menu() {
		bws_general_menu();
		$settings_page = add_submenu_page( 'bws_panel', __( 'Captcha Plus Settings', 'captcha-plus' ), 'Captcha Plus', 'manage_options', 'captcha-plus.php', 'cptchpls_settings_page' );
...
if ( ! function_exists ( 'cptchpls_init' ) ) {
	function cptchpls_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $cptchpls_plugin_info, '3.8' );
...
if ( ! function_exists( 'cptchpls_create_table' ) ) {
	function cptchpls_create_table() {
		...
		/* remove unnecessary columns from the 'whitelist' table */
...
if ( ! function_exists( 'cptchpls_settings' ) ) {
	function cptchpls_settings() {
		...
		$cptchpls_option_defaults = array(
			 ...
			 * @since 1.1.4
			 */
			'time_limit_notice'					=> __( 'Time limit is exhausted. Please reload CAPTCHA.', 'captcha-plus' )
		);
		...
		if ( isset( $_REQUEST['cptchpls_form_submit'] ) && check_admin_referer( $plugin_basename, 'cptchpls_nonce_name' ) ) {
			...
			if ( ! isset( $_GET['action'] ) ) {
				...
				if ( 1 == $cptchpls_options['cptchpls_difficulty_image'] ) {
					$package_list = $wpdb->get_results( "SELECT `id` FROM `{$wpdb->base_prefix}cptch_packages` LIMIT 1;", ARRAY_A );
				...
				if ( empty( $cptchpls_options['used_packages'] ) && 1 == $cptchpls_options['cptchpls_difficulty_image'] )
				...
				$cptchpls_options['time_limit_notice'] 				= isset( $_REQUEST['cptchpls_time_limit_notice'] ) ? stripslashes( esc_html( $_REQUEST['cptchpls_time_limit_notice'] ) ) : '';
			...
			if ( empty( $error ) ) {
				$cptchpls_options = apply_filters( 'cptch_before_save_options', $cptchpls_options );
					...
					<form class="bws_form" method="post" action="">
						<table class="form-table">
							<?php if ( ! isset( $_GET['action'] ) ) { ?>
								...
								<?php do_action( 'cptch_settings_page_action', $cptchpls_options ); ?>
							<?php } else {
								if ( ! empty( $package_list ) ) { ?>
									<tr class="cptchpls_packages">
										...
										<td>
											<div id="cptchpls_package_list">
												<ul>
													<?php foreach ( $package_list as $pack ) {
														$checked = in_array( $pack['id'], $cptchpls_options['used_packages'] ) ? ' checked="checked"': ''; ?>
														<li>
															<span><input id="cptchpls_used_packages_<?php echo $pack['id']; ?>" type="checkbox" name="cptchpls_used_packages[]" value="<?php echo $pack['id']; ?>"<?php echo $checked; ?> /></span>
															<span><label for="cptchpls_used_packages_<?php echo $pack['id']; ?>"><img src="<?php echo "{$packages_url}/{$pack['folder']}/{$pack['image']}"; ?>" title="<?php echo $pack['name']; ?>"/></label></span>
															<span><label for="cptchpls_used_packages_<?php echo $pack['id']; ?>"><?php echo $pack['name']; ?></label></span>
														</li>
													<?php } ?>
												</ul>
											</div>
								...
								<tr valign="top">
									...
									<td colspan="2">
										<p>
											<i><?php _e( "Error", 'captcha-plus' ); ?></i>
											<span class="bws_help_box dashicons dashicons-editor-help">
												<span class="bws_hidden_help_text" style="width: 200px;"><?php _e( 'This messages will be displayed, if the CAPTCHA answer has not passed the verification', 'captcha-plus' ); ?>.</span>
											</span>
										</p>
										<p><input class="cptchpls_settings_input" type="text" name="cptchpls_error_empty_value" value="<?php echo $cptchpls_options['cptchpls_error_empty_value']; ?>" maxlength="100" />&nbsp;<?php _e( 'If the CAPTCHA field is empty', 'captcha-plus' ); ?></p>
										<p><input class="cptchpls_settings_input" type="text" name="cptchpls_error_incorrect_value" value="<?php echo $cptchpls_options['cptchpls_error_incorrect_value']; ?>" maxlength="100" />&nbsp;<?php _e( 'If the CAPTCHA is incorrect', 'captcha-plus' ); ?></p>
										<p><input class="cptchpls_settings_input" type="text" name="cptchpls_error_time_limit" value="<?php echo $cptchpls_options['cptchpls_error_time_limit']; ?>" maxlength="100" />&nbsp;<?php _e( 'If the time limit is exhausted', 'captcha-plus' ); ?></p>
										<p>
											<i><?php _e( "Info", 'captcha-plus' ); ?></i>
											<span class="bws_help_box dashicons dashicons-editor-help">
												<span class="bws_hidden_help_text" style="width: 200px;"><?php _e( 'This messages will be displayed inside of the CAPTCHA', 'captcha-plus' ); ?>.</span>
											</span>
										</p>
										<p><input class="cptchpls_settings_input" type="text" name="cptchpls_whitelist_message" value="<?php echo $cptchpls_options['whitelist_message']; ?>" maxlength="100"  />&nbsp;<?php _e( 'If the user IP is added to the whitelist (this message will be displayed instead of the CAPTCHA)', 'captcha-plus' ); ?></p>
										<p><input class="cptchpls_settings_input" type="text" maxlength="100" name="cptchpls_time_limit_notice" value="<?php echo $cptchpls_options['time_limit_notice']; ?>" /> <?php _e( 'If the time limit is exhausted (this message will be displayed above the CAPTCHA)', 'captcha-plus' ); ?></p>
									</td>
								</tr>
							<?php } ?>
						</table>
...
if ( ! function_exists( 'cptchpls_display_captcha' ) ) {
	function cptchpls_display_captcha( $class_name = "", $input_name = 'cptchpls_number' ) {
		...
		foreach ( $operand_formats as $key => $format ) {
			switch ( $format ) {
				case 'input':
					$operand[] = '<input id="cptchpls_input_' . $id_postfix . '" class="cptchpls_input ' . $class_name . '" type="text" autocomplete="off" name="' . $input_name . '" value="" maxlength="2" size="2" aria-required="true" required="required" style="margin-bottom:0;display:inline;font-size: 12px;width: 40px;" />';
					break;
				case 'word':
					$operand[] = cptchpls_generate_value( $array_math_expretion[ $key ] );
					break;
				case 'image':
					$array_key = mt_rand( 0, abs( count( $cptchpls_options['used_packages'] ) - 1 ) );
					$operand[] =
							empty( $cptchpls_options['used_packages'][ $array_key ] )
						?
							cptchpls_generate_value( $array_math_expretion[ $key ] )
						:
							cptchpls_get_image( $array_math_expretion[ $key ], $key, $cptchpls_options['used_packages'][ $array_key ], $use_only_words );
					break;
				case 'number':
				default:
		...
		$hidden_result_name = $input_name == 'cptchpls_number' ? 'cptchpls_result' : $input_name . '-cptchpls_result';

		return
			cptchpls_add_time_limit_notice( $id_postfix ) .
			cptchpls_add_scripts() .
			'<span class="cptchpls_wrap">
				...
				cptchpls_add_reload_button( !! $cptchpls_options['display_reload_button'] ) .
			'</span>';
	}
}
 ...
 */
if ( ! function_exists( 'cptchpls_add_scripts' ) ) {
	function cptchpls_add_scripts () {
		global $cptchpls_options;

		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			return
					defined( 'CPTCHPLS_RELOAD_AJAX' )
				?
					''
				:
					/*
					 ...
					 */
					'<script class="cptchpls_to_remove" type="text/javascript">
						(function( d, tag, id ) {
						...
						})( document, "script", "cptchpls_script_loader" );
					</script>';
		} elseif ( ! wp_script_is( 'cptchpls_front_end_script', 'registered' ) ) {
			wp_register_script( 'cptchpls_front_end_script', plugins_url( 'js/front_end_script.js' , __FILE__ ), array( 'jquery' ), false, true );
			add_action( 'wp_footer', 'cptchpls_front_end_scripts' );
			if (
				'1' == $cptchpls_options['cptchpls_login_form'] ||
				'1' == $cptchpls_options['cptchpls_lost_password_form'] ||
				'1' == $cptchpls_options['cptchpls_register_form']
			)
				add_action( 'login_footer', 'cptchpls_front_end_scripts' );
		}
		return '';
	}
}
 ...
 */
if ( ! function_exists( 'cptchpls_add_time_limit_notice' ) ) {
	function cptchpls_add_time_limit_notice( $id_postfix ) {
		global $cptchpls_options;

		if ( ! $cptchpls_options['use_time_limit'] || ! $cptchpls_options['time_limit'] )
			return '';
		$id = "cptchpls_time_limit_notice_{$id_postfix}";
		return
			'<script class="cptchpls_to_remove">
				(function( timeout ) {
					setTimeout(
						function() {
				...
				})(' . $cptchpls_options['time_limit'] . '000);
			</script>
			<span id="' . $id . '" class="cptchpls_time_limit_notice cptchpls_to_remove">' . $cptchpls_options['time_limit_notice'] . '</span>';
	}
}
 ...
 */
if ( ! function_exists( 'cptchpls_add_reload_button' ) ) {
	function cptchpls_add_reload_button( $add_button ) {
		return
				$add_button
			?
...
if ( ! function_exists( 'cptchpls_get_image' ) ) {
	function cptchpls_get_image( $value, $place, $package_id, $use_only_words ) {
		...
		if ( file_exists( $src ) ) {
			...
			return isset( $image_data['mime'] ) && ! empty( $image_data['mime'] ) ? '<img class="cptchpls_img ' . $class . '" src="data:' . $image_data['mime'] . ';base64,'. base64_encode( file_get_contents( $src ) ) . '" alt="image"/>' :  cptchpls_generate_value( $value, $use_only_words );
		} else {
			return cptchpls_generate_value( $value, $use_only_words );
		}
	}
...
if ( ! function_exists( 'cptchpls_display_captcha_custom' ) ) {
	function cptchpls_display_captcha_custom( $class_name = "", $input_name = 'cptchpls_number' ) {
		...
		$content = $cptchpls_ip_in_whitelist ? '<label class="cptchpls_whitelist_message">' . $cptchpls_options['whitelist_message'] . '</label>' : cptchpls_display_captcha( $class_name, $input_name );
...
if ( ! function_exists( 'cptchpls_front_end_scripts' ) ) {
	function cptchpls_front_end_scripts() {
		...
		if (
			wp_script_is( 'cptchpls_front_end_script', 'registered' ) &&
			! wp_script_is( 'cptchpls_front_end_script', 'enqueued' )
		) {
			wp_enqueue_script( 'cptchpls_front_end_script' );
			$args = array(
				'nonce'        => wp_create_nonce( 'cptchpls', 'cptchpls_nonce' ),
				'ajaxurl'      => admin_url( 'admin-ajax.php' ),
				'enlarge'      => $cptchpls_options['enlarge_images']
			);
			wp_localize_script( 'cptchpls_front_end_script', 'cptchpls_vars', $args );
		}
	}
}
...
if ( ! function_exists ( 'cptchpls_admin_head' ) ) {
	function cptchpls_admin_head() {
		if ( isset( $_REQUEST['page'] ) && 'captcha-plus.php' == $_REQUEST['page'] ) {
			wp_enqueue_style( 'cptchpls_stylesheet', plugins_url( 'css/style.css', __FILE__ ) );
			wp_enqueue_script( 'cptchpls_script', plugins_url( 'js/script.js', __FILE__ ), array( 'jquery', 'jquery-ui-resizable' ) );
...
if ( ! function_exists( 'cptchpls_reload' ) ) {
	function cptchpls_reload() {
		...
		if ( ! defined( 'CPTCHPLS_RELOAD_AJAX' ) )
			define( 'CPTCHPLS_RELOAD_AJAX', true );
...
$cptchpls_time            = time();
$cptchpls_options         = get_option( 'cptchpls_options' );
if ( empty( $cptchpls_options ) && ! is_admin() ) {
	cptchpls_settings();
	$cptchpls_options = get_option( 'cptchpls_options' );
}
